...
export default async function install(spec = 'latest', apiKey, apiKeySource) {
    ...
    if (apiKey) {
        ...
        await exec(path.join(cachePath, process.platform === 'win32' ? 'nuget.exe' : 'nuget'), args, { silent: true });
